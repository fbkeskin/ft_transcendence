      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-slate-800 rounded-xl p-6 shadow-lg border border-slate-700">
                <div class="flex flex-col items-center">
                    ...
                    <div class="grid grid-cols-3 gap-2 w-full text-center">
                        ...
                        <div class="bg-slate-900 p-2 rounded-lg border border-yellow-600/30 flex flex-col justify-center relative overflow-hidden"><div class="absolute inset-0 bg-yellow-500/10"></div><span class="block text-lg font-bold text-yellow-400 z-10" id="user-cups">0</span><span class="text-[10px] text-yellow-200/70 uppercase tracking-tighter z-10">KUPA</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-slate-800 rounded-xl p-4 shadow-lg border border-slate-700 min-h-[300px] flex flex-col">
                ...
                <div class="flex border-b border-gray-700 mb-2">
                    <button id="tab-lobby" class="flex-1 py-2 text-xs font-bold text-white border-b-2 border-green-500 transition">🌐 LOBBY</button>
                ...
                <div id="view-friends" class="flex-1 overflow-y-auto pr-1 hidden">
                    <div id="pending-requests" class="mb-2 hidden">
                        <p class="text-[10px] text-gray-400 uppercase font-bold mb-1">İstekler</p>
                        <div id="pending-list" class="space-y-1"></div>
                    </div>
                    <p class="text-[10px] text-gray-400 uppercase font-bold mb-1">Listem</p>
                    <div id="friends-list" class="space-y-2">
                        <p class="text-gray-500 text-xs text-center py-2">Arkadaşın yok :(</p>
                    </div>
                </div>
        ...
        <div class="md:col-span-2 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <button class="game-btn h-32 bg-gradient-to-br from-orange-600 to-red-700 rounded-xl flex flex-col items-center justify-center hover:scale-105 transition shadow-lg group relative overflow-hidden"><span class="text-4xl mb-2 group-hover:rotate-12 transition">🏆</span><span class="font-bold text-xl">Turnuva</span></button>
            </div>
            ...
            <div class="bg-slate-800 rounded-xl border border-slate-700 shadow-lg flex flex-col h-96 relative overflow-hidden">
                <div class="flex border-b border-gray-700 bg-slate-900/50">
                    <button id="tab-hist-matches" class="flex-1 py-4 text-sm font-bold text-white border-b-2 border-indigo-500 bg-slate-800 transition hover:bg-slate-700">📊 1v1 MAÇLAR</button>
                    <button id="tab-hist-tournaments" class="flex-1 py-4 text-sm font-bold text-gray-400 border-b-2 border-transparent hover:text-white hover:bg-slate-700 transition">🏆 TURNUVALAR</button>
                </div>
                <div class="overflow-y-auto flex-1 pr-2 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent p-1">
                    ...
                    <table id="table-tournaments" class="w-full text-sm text-left text-gray-400 hidden"><thead class="text-xs text-gray-300 uppercase bg-slate-900/50 sticky top-0"><tr><th class="px-4 py-3 text-center">#</th><th class="px-4 py-3">Katılımcılar</th><th class="px-4 py-3 text-center">Şampiyon</th><th class="px-4 py-3 text-center">Durum</th><th class="px-4 py-3 text-right">Tarih</th></tr></thead><tbody id="tournament-history-body" class="divide-y divide-slate-700"></tbody></table>
                </div>
            </div>
        </div>
      </div>
  ...
  init: async () => {
    try {
        ...
        function renderFriendsList() { 
            ...
            if (friends.length === 0) {
                friendsContainer.innerHTML = `<p class="text-gray-500 text-xs text-center py-2">Henüz arkadaşın yok.</p>`;
            } else {
                friendsContainer.innerHTML = friends.map((f: any) => {
                    ...
                    <div class="flex items-center justify-between bg-slate-700/30 p-2 rounded border border-slate-600 hover:bg-slate-700 transition group">
                        ...
                        <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                            ${isOnline ? `<button class="invite-btn text-[10px] bg-indigo-600 hover:bg-indigo-500 px-2 py-1 rounded text-white" data-id="${f.id}">VS</button>` : ''}
                            <button class="remove-friend-btn text-[10px] bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white px-2 py-1 rounded" data-id="${f.id}">Sil</button>
                        </div>
                    </div>
                    `;
                }).join('');
        ...
        function renderLobby() {
            ...
            if (onlineList.length === 0) {
                listContainer.innerHTML = `<p class="text-gray-500 text-xs text-center py-2">Kimse yok :(</p>`;
                return;
            }
            ...
            listContainer.innerHTML = onlineList.map(u => {
                ...
                if (!isFriend) {
                    if (isSent) {
                        actionBtn = `<button class="text-[10px] bg-gray-600 px-2 py-1 rounded text-gray-300 cursor-not-allowed" disabled>İstek Yollandı</button>`;
                    } else {
                        actionBtn = `<button class="add-friend-btn text-[10px] bg-emerald-600 hover:bg-emerald-500 px-2 py-1 rounded text-white" data-id="${u.id}">+Arkadaş</button>`;
                    }
                }
                ...
                <li class="flex items-center justify-between bg-slate-800 p-2 rounded border border-slate-700">
                    ...
                    <div class="flex gap-1">
                        ${actionBtn}
                        <button class="invite-btn text-[10px] bg-indigo-600 hover:bg-indigo-500 px-2 py-1 rounded text-white" data-id="${u.id}">Vs At</button>
                    </div>
                </li>`;
            }).join('');
        };
        ...
        document.body.addEventListener('click', async (e) => {
            ...
            if (target.classList.contains('add-friend-btn')) {
                ...
                try {
                    ...
                    if (err.message && err.message.includes('Zaten')) {
                         Modal.alert("Bilgi", "Bu kişiye zaten istek gönderdin.");
                         // Yine de listede kalsın
                         sentRequestsLocal.add(id);
                         renderLobby();
                    } else {
                         Modal.alert("Hata", err.message); 
                    }
                }
            }
            ...
            if (target.classList.contains('remove-friend-btn') || target.classList.contains('reject-btn')) {
                const id = Number(target.getAttribute('data-id'));
                const isConfirm = await Modal.confirm("Emin misin?", "Bu işlemi onaylıyor musun?");
                if (isConfirm) {
                    try {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (btn && !btn.hasAttribute('disabled')) {
             ...
             if(targetId) { socketService.sendGameInvite(targetId); Modal.alert("📨 Davet yollandı", "Bekleniyor..."); }
        }
    });

    socketService.onIncomingInvite(async (data) => {
        Modal.closeAll();
        const accepted = await Modal.confirm("⚔️ OYUN İSTEĞİ!", `<strong>${data.senderName}</strong> seni çağırıyor!`);
        socketService.respondToInvite(data.senderId, accepted);
    });
    socketService.onInviteRejected((data) => { Modal.closeAll(); Modal.alert("❌ Reddedildi", `${data.rejecterName} kabul etmedi.`); });
    socketService.onGameStart((data) => {
        ...
        Modal.alert("🚀 OYUN BAŞLIYOR!", `Rakip: ${data.opponent}`).then(() => navigate('/game/online'));
    });
  }
};
...
function renderMatchHistory(user: any) {
    ...
    if (allGames.length === 0) matchBody.innerHTML = `<tr><td colspan="5" class="text-center py-8 text-gray-500 italic">${lang.t('dash_no_match')}</td></tr>`;
        ...
        let opponentName = isPlayer1 ? (game.player2 ? game.player2.username : (game.guestName || 'Misafir')) : (game.player1 ? game.player1.username : 'Rakip');
        ...
        matchBody.innerHTML += `<tr class="border-b border-slate-700/50 hover:bg-slate-700/30 transition"><td class="px-4 py-3 text-center text-gray-600 font-mono text-xs">${index + 1}</td><td class="px-4 py-3 font-medium text-slate-200">${opponentName}</td><td class="px-4 py-3 font-mono text-indigo-300 font-bold tracking-widest">${myScore} - ${enemyScore}</td><td class="px-4 py-3">${resultBadges}</td><td class="px-4 py-3 text-right text-gray-500 text-xs">${new Date(game.createdAt).toLocaleDateString('tr-TR')}</td></tr>`;
    });
...
function renderTournamentHistory(tournaments: any[], user: any) {
    ...
    if (tournaments.length === 0) tournamentBody.innerHTML = `<tr><td colspan="5" class="text-center py-8 text-gray-500 italic">Henüz bir turnuva düzenlenmedi.</td></tr>`;
    else tournaments.forEach((t: any, index: number) => {
        const isChampion = t.winner === user.username;
        let statusBadge = isChampion ? `<span class="bg-yellow-500/10 text-yellow-400 px-2 py-1 rounded text-xs font-bold border border-yellow-500/20">🏆 ZAFER</span>` : `<span class="bg-slate-700 text-gray-400 px-2 py-1 rounded text-xs border border-slate-600">KATILIM</span>`;
        tournamentBody.innerHTML += `<tr class="border-b border-slate-700/50 hover:bg-slate-700/30 transition"><td class="px-4 py-3 text-center text-gray-600 font-mono text-xs">${index + 1}</td><td class="px-4 py-3 text-xs text-gray-400 max-w-[150px] truncate">${t.player1}, ${t.player2}...</td><td class="px-4 py-3 text-center font-bold text-indigo-300">${t.winner}</td><td class="px-4 py-3 text-center">${statusBadge}</td><td class="px-4 py-3 text-right text-gray-500 text-xs">${new Date(t.createdAt).toLocaleDateString('tr-TR')}</td></tr>`;
    });
}