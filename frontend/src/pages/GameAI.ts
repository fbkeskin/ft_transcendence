...
export const GameAI = {
  ...
  init: async () => {
    ...
    function startAILogic() {
        // SUBJECT: Refresh view once per second
    ...
    function update() {
        ...
        const tolerance = 15;
        aiKeys.w = paddleCenter > aiDecisionY + tolerance;
        aiKeys.s = paddleCenter < aiDecisionY - tolerance;

        const paddleSpeed = 8; // AI hızı oyuncudan (9) biraz daha yavaş
        if (aiKeys.w && player1.y > 0) player1.y -= paddleSpeed;
        if (aiKeys.s && player1.y < canvas.height - PADDLE_HEIGHT) player1.y += paddleSpeed;

        // Oyuncu Hareketi
        const playerSpeed = 9;
        if ((keys['ArrowUp'] || keys['TouchUp']) && player2.y > 0) player2.y -= playerSpeed;
        if ((keys['ArrowDown'] || keys['TouchDown']) && player2.y < canvas.height - PADDLE_HEIGHT) player2.y += playerSpeed;
        ...
        if (checkCollision(ball, player1)) { 
            ...
            if (Math.abs(ball.speedX) > MAX_SPEED) ball.speedX = MAX_SPEED;
    ...
    const handleKeyDown = (e: KeyboardEvent) => { 
        ...
        if(e.key === 'Escape') { navigate('/dashboard'); return; }
        keys[e.key] = true; 
    };
