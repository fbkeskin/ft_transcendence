...
export const TournamentBracket = {
  render: () => {
    ...
    if (!data) return `<div class="text-white text-center mt-20">${lang.t('tour_not_found')}</div>`;
    ...
    return `
      <div class="min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-4">
        ...
        <h1 class="text-4xl font-black mb-8 text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500">
            ${isFinished ? lang.t('tour_bracket_winner') : lang.t('tour_bracket_title')}
        </h1>
        ...
        <div class="flex flex-col gap-8 w-full max-w-4xl">
            ...
            <div class="flex justify-between gap-10">
                <div class="flex-1 bg-slate-800 p-4 rounded-xl border ${matches[0].winner ? 'border-green-500/50' : 'border-slate-700'} text-center relative">
                    <span class="text-xs text-gray-500 uppercase">${lang.t('tour_bracket_semi1')}</span>
                    <div class="font-bold text-xl my-2">${escapeHTML(matches[0].p1)} vs ${escapeHTML(matches[0].p2)}</div>
                    ${matches[0].winner ? `<div class="text-green-400 text-sm">${lang.t('tour_bracket_winner_label')}: ${escapeHTML(matches[0].winner)}</div>` : `<div class="text-yellow-500 text-sm">${lang.t('tour_bracket_waiting')}</div>`}
                </div>
                ...
                <div class="flex-1 bg-slate-800 p-4 rounded-xl border ${matches[1].winner ? 'border-green-500/50' : 'border-slate-700'} text-center relative">
                    <span class="text-xs text-gray-500 uppercase">${lang.t('tour_bracket_semi2')}</span>
                    <div class="font-bold text-xl my-2">${escapeHTML(matches[1].p1)} vs ${escapeHTML(matches[1].p2)}</div>
                    ${matches[1].winner ? `<div class="text-green-400 text-sm">${lang.t('tour_bracket_winner_label')}: ${escapeHTML(matches[1].winner)}</div>` : `<div class="text-yellow-500 text-sm">${lang.t('tour_bracket_waiting')}</div>`}
                </div>
            </div>
            ...
            <div class="flex justify-center">
                <div class="w-1/2 bg-slate-800 p-6 rounded-xl border-2 ${matches[2].winner ? 'border-yellow-500 shadow-[0_0_20px_rgba(234,179,8,0.3)]' : 'border-indigo-500'} text-center relative">
                    <span class="text-xs text-indigo-400 uppercase font-bold tracking-widest">${lang.t('tour_bracket_final')}</span>
                    <div class="font-bold text-2xl my-4">
                        ${escapeHTML(matches[2].p1 || '?')} vs ${escapeHTML(matches[2].p2 || '?')}
                    </div>
                    ${champion ? `
                        <div class="text-4xl mt-2 animate-bounce">🏆 ${escapeHTML(champion)} 🏆</div>
                    ` : `<div class="text-gray-500 text-sm">${lang.t('tour_bracket_final_waiting')}</div>`}
                </div>
            </div>
        ...
        <div class="mt-12">
            ${isFinished ? `
                <button id="finish-tournament" class="bg-gray-700 hover:bg-gray-600 px-8 py-3 rounded-xl font-bold transition">
                    ${lang.t('tour_bracket_finish')}
                </button>
            ` : `
                <div class="text-center">
                    <p class="text-gray-400 mb-2 text-sm">${lang.t('tour_bracket_next_match')}</p>
                    <button id="start-match-btn" class="bg-gradient-to-r from-indigo-600 to-blue-600 hover:scale-105 px-10 py-4 rounded-xl font-bold text-xl transition shadow-lg">
                        ▶️ ${escapeHTML(currentMatch.p1)} vs ${escapeHTML(currentMatch.p2)} ${lang.t('tour_bracket_start_match')}
                    </button>
                </div>
            `}
        </div>
