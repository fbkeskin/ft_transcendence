...
class SocketService {
  ...
  connect() {
    ...
    updateStatus(status: 'AVAILABLE' | 'BUSY' | 'IN_GAME' | 'WAITING') {
    ...
    getPendingInvites() { return this.pendingInvites; }
    ...
    subscribeToEvent(event: string, callback: (data: any) => void) {
        ...
        if (this.socket && this.socket.connected) this.socket.on(event, callback);
        return () => {
    ...
    public notifyListeners() { this.listeners.forEach(callback => callback()); }
    ...
    confirmReady(opponentId: number) { this.socket?.emit('confirm_ready', { opponentId }); }
    ...
    sendPaddleMove(y: number) { if (this.socket && this.currentOpponentId) this.socket.emit('game_paddle_move', { y, opponentId: this.currentOpponentId }); }
    sendBallUpdate(ballX: number, ballY: number, score1: number, score2: number) {
        if (this.socket && this.currentOpponentId) this.socket.emit('game_ball_update', { ballX, ballY, score1, score2, opponentId: this.currentOpponentId });
    }
    sendGameOver(winner: string) { if (this.socket && this.currentOpponentId) this.socket.emit('game_over_signal', { winner, opponentId: this.currentOpponentId }); }
